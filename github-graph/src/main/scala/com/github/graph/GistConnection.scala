...
object GistConnection {
  ...
  /** A list of edges.
    */
  def edges[A](innerSelection: SelectionBuilder[GistEdge, A]): SelectionBuilder[GistConnection, Option[List[Option[A]]]] =
    _root_.caliban.client.SelectionBuilder.Field("edges", OptionOf(ListOf(OptionOf(Obj(innerSelection)))))

  /** A list of nodes.
    */
  def nodes[A](innerSelection: SelectionBuilder[Gist, A]): SelectionBuilder[GistConnection, Option[List[Option[A]]]] =
