...
object UnsubscribedEvent {
  ...
  ): SelectionBuilder[UnsubscribedEvent, Option[A]] = _root_.caliban.client.SelectionBuilder.Field(
    "actor",
    OptionOf(
      ChoiceOf(
        Map(
  ...
  ): SelectionBuilder[UnsubscribedEvent, A] = _root_.caliban.client.SelectionBuilder.Field(
    "subscribable",
    ChoiceOf(
      Map(
        "Commit"         -> onCommit,
