...
object DeploymentConnection {
  ...
  /** A list of edges.
    */
  def edges[A](innerSelection: SelectionBuilder[DeploymentEdge, A]): SelectionBuilder[DeploymentConnection, Option[List[Option[A]]]] =
    _root_.caliban.client.SelectionBuilder.Field("edges", OptionOf(ListOf(OptionOf(Obj(innerSelection)))))

  /** A list of nodes.
    */
  def nodes[A](innerSelection: SelectionBuilder[Deployment, A]): SelectionBuilder[DeploymentConnection, Option[List[Option[A]]]] =
    _root_.caliban.client.SelectionBuilder.Field("nodes", OptionOf(ListOf(OptionOf(Obj(innerSelection)))))

  /** Information to aid in pagination.
    */
  def pageInfo[A](innerSelection: SelectionBuilder[PageInfo, A]): SelectionBuilder[DeploymentConnection, A] =
