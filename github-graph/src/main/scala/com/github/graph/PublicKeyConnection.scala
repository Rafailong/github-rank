...
object PublicKeyConnection {
  ...
  /** A list of edges.
    */
  def edges[A](innerSelection: SelectionBuilder[PublicKeyEdge, A]): SelectionBuilder[PublicKeyConnection, Option[List[Option[A]]]] =
    _root_.caliban.client.SelectionBuilder.Field("edges", OptionOf(ListOf(OptionOf(Obj(innerSelection)))))

  /** A list of nodes.
    */
  def nodes[A](innerSelection: SelectionBuilder[PublicKey, A]): SelectionBuilder[PublicKeyConnection, Option[List[Option[A]]]] =
    _root_.caliban.client.SelectionBuilder.Field("nodes", OptionOf(ListOf(OptionOf(Obj(innerSelection)))))

  /** Information to aid in pagination.
    */
  def pageInfo[A](innerSelection: SelectionBuilder[PageInfo, A]): SelectionBuilder[PublicKeyConnection, A] =
